...
# Prefer the key from .env; fall back to a text box if it isn't set.
openai_api_key = os.getenv("OPENAI_API_KEY")

if openai_api_key:
    st.caption("Using OPENAI_API_KEY from .env")
else:
...
@st.cache_resource
# This decorator tells Streamlit to load the SQL Agent only once and cache it.
# Creating an LLM + SQL Agent is expensive, so caching avoids rebuilding it every time the user sends a message.
def load_agent():
    llm = ChatOpenAI(
        model="gpt-4.1",
        temperature=0
